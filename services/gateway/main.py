...
queue_breaker = CircuitBreaker(
    name="redis_queue",
    config=CircuitBreakerConfig(failure_threshold=3, timeout=30, success_threshold=2),
)
...
async def health_check():
    try:
        ...
        return JSONResponse(
            status_code=503,
            content={"status": "unhealthy", "service": "gateway", "error": str(e)},
        )
...
async def submit_job(job_type: str, payload: dict | None = None):
    ...
    try:
        ...
        return {"job_id": job.job_id, "status": job.status, "message": "Job submitted successfully"}
    except Exception as e:
        queue_breaker.record_failure()
